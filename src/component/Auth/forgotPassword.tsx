...
export default function ForgetPassword() {
  ...
  async function handleForget(values: { email: string }) {
    try {
      setLoading(true);
      const { data } = await axios.post("https://node-hr.vercel.app/auth/forgot-password",
    { email: values.email },
  ...
  return <>
    <motion.div initial={{ opacity: 0  }} animate={{ opacity: 1  }} transition={{ duration: 1.10, ease: "easeOut" }} style={{ width: "100%" }}>
    <Box sx={{display: "flex",justifyContent: "center",alignItems: "center",px: { lg: 10 },overflow: "hidden",minHeight: "80vh",}}>
     ...
     {/* Left Side Hero */}
        <motion.div>
          <Box sx={{flex: {sx:"0 0 0%" , md: "0 0 60%"},display: { xs: "none", md: "flex" },alignItems: "center",justifyContent: "center",background: "#202020",
            color: "#fff",boxShadow: "0 4px 12px rgba(0,0,0,0.3)",flexDirection: "column",textAlign: "center",p: 4,height: "640px",}}>
            ...
            <Typography variant="body1" sx={{ mt: 2, maxWidth: 400 }}>Sign in to continue and access your personalized dashboard.</Typography>
          </Box>
        </motion.div>
        ...
        {/* Right Side Form */}
         ...
         <Paper sx={{p: 4,width: "100%",position: "relative",height: "640px",background:'#2c2c2c'}}>
            ...
            <Box sx={{ transform: "translateY(80px)" }}>
              <Typography variant="h5" align="center" gutterBottom sx={{ pt: 5,color:'white' }}>
                Forget Password
              </Typography>
            ...
            <form onSubmit={formik.handleSubmit}>
              ...
              {/* API Error */}
              {errorApi && (
                <Typography sx={{ transform: "translateY(10px)" }} color="error" variant="body2" mt={1}>
                  {errorApi}
                </Typography>
              )}
              ...
              <Button
                ...
                fullWidth
                variant="contained"
                sx={{mt: 3,py: 1.2,borderRadius: 2,background: loading ?  '#313131ff' : "linear-gradient(135deg, #1976d2, #42a5f5)"}}
                disabled={loading}>
                {loading ? (
